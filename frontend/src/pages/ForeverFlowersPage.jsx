...
const TAGS = ['All', 'romantic', 'handmade', 'premium', 'aesthetic', 'bright', 'minimal'];

function Stars({ rating }) {
  return (
    <span style={{ color: '#e8476a', fontSize: 12 }}>
      ...
      <div style={{ position: 'relative', overflow: 'hidden', aspectRatio: '1/1', background: '#fdf6f0' }}>
        ...
        <button
          ...
          style={{
            ...
            background: wished ? '#fce8ed' : 'rgba(255,252,250,0.92)',
            border: wished ? '1px solid #f7b8c8' : '1px solid rgba(232,71,106,0.18)',
            borderRadius: '50%', width: 34, height: 34, fontSize: 17,
            cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center',
            color: wished ? '#e8476a' : '#e8909f', transition: 'all 0.2s',
          }}
        >
          {wished ? '♥' : '♡'}
        </button>
        {item.rating >= 4.9 && (
          <div style={{ position: 'absolute', top: 10, left: 10, background: '#fce8ed', color: '#c0344f', border: '1px solid #f7b8c8', fontSize: 10, fontWeight: 800, padding: '3px 10px', borderRadius: 20 }}>
            Top Rated
          </div>
        )}
      </div>
      ...
      <div style={{ padding: '16px', flex: 1, display: 'flex', flexDirection: 'column', gap: 6 }}>
        ...
        <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', marginTop: 4 }}>
          {item.tags?.slice(0, 3).map(t => (
            <span key={t} style={{ fontSize: 10, background: '#fdf0f5', color: '#c0344f', border: '1px solid #f7b8c8', borderRadius: 20, padding: '2px 8px', fontWeight: 600 }}>{t}</span>
          ))}
        </div>
        ...
        <button
          ...
          style={{
            ...
            boxShadow: inCart ? 'none' : '0 4px 16px rgba(232,71,106,0.35)',
            transition: 'all 0.2s',
          }}
        >
...
export default function ForeverFlowersPage() {
  ...
  const handleAdd = (item) => {
    addToCart({ _id: item._id, name: item.name, pricePerStem: item.price, image: item.image, category: 'flowers' });
    setToast(item.name);
  ...
  return (
    <div style={{ fontFamily: "'Segoe UI',system-ui,sans-serif", background: 'linear-gradient(145deg,#fff0f5,#fde8f0,#f8e0f8)', minHeight: '100vh' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg,#2c1a2e,#4a1030,#2d1a2e)', padding: '60px 24px 48px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        <h1 style={{ margin: '0 0 14px', fontSize: 'clamp(2rem,5vw,3.2rem)', fontWeight: 900, color: '#fff', lineHeight: 1.15, letterSpacing: -1 }}>
          Forever Flowers 🌸<br />
          <span style={{ background: 'linear-gradient(90deg,#f7b8c8,#e8476a,#c9a84c)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
            Always Blooming
          </span>
        </h1>
        <p style={{ margin: '0 auto', fontSize: 15, color: 'rgba(253,240,245,0.75)', maxWidth: 480, lineHeight: 1.7 }}>
          Handcrafted crochet bouquets that never wilt — the perfect gift that lasts forever
        </p>
        <div style={{ display: 'flex', justifyContent: 'center', gap: 32, marginTop: 36 }}>
          {[['11+', 'Designs'], ['100%', 'Handmade'], ['Forever', 'Lasting']].map(([n, l]) => (
            <div key={l} style={{ textAlign: 'center' }}>
              <div style={{ fontSize: 22, fontWeight: 900, color: '#f7b8c8' }}>{n}</div>
              <div style={{ fontSize: 11, color: 'rgba(253,240,245,0.5)', marginTop: 2 }}>{l}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ maxWidth: 1160, margin: '0 auto', padding: '40px 22px 80px' }}>
            ...
            {TAGS.map(t => (
              <button key={t} onClick={() => setActiveTag(t)}
                style={{
                  ...
                  border: activeTag === t ? '2px solid #e8476a' : '1.5px solid #f0e8eb',
                  background: activeTag === t ? '#fce8ed' : '#fff',
                  color: activeTag === t ? '#c0344f' : '#9a8a94',
                  boxShadow: activeTag === t ? '0 2px 12px rgba(232,71,106,0.2)' : 'none',
                }}>
                {t}
              </button>
            ))}
        ...
        <p style={{ margin: '0 0 24px', fontSize: 12.5, color: '#9a8a94' }}>
          Showing <strong style={{ color: '#2c1a2e' }}>{filtered.length}</strong> bouquets
        </p>

        {loading && (
          <div style={{ textAlign: 'center', padding: '60px', color: '#e8476a', fontSize: 15 }}>🌸 Loading forever flowers...</div>
        )}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(240px,1fr))', gap: 20 }}>
          {filtered.map(item => (
            <ProductCard
              ...
              onWish={() => toggleWish({ _id: item._id, name: item.name, price: item.price, image: item.image, category: 'flowers' })}
            />
          ))}
        </div>
      </div>
